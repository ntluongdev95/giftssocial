...
import { OCCASIONS, featuredOccasion } from '@/lib/occasions';
...
function mergedTemplates(occasion: Occasion): MergedTemplate[] {
  ...
  const legacy: MergedTemplate[] = (occasion.templates ?? [])
    .filter(t => !TEMPLATE_REGISTRY[t.id])
    .map(t => ({
      id: t.id, name: t.name, description: t.description, emoji: t.emoji,
      thumbnailBg: t.thumbnailBg, premium: t.premium, coins: t.coins,
      previewVideo: t.videoUrl, hasComponent: false,
    }));
...
export function SendKissModal({ onClose, onSent, defaultReceiverId, inline = false, hideHeader = false }: { onClose: () => void; onSent: () => void; defaultReceiverId?: string | null; inline?: boolean; hideHeader?: boolean }) {
  ...
  const doSend = async (overrideReceiverCoords?: { lat: number; lng: number }) => {
    ...
    try {
      ...
      // Only send template_id if it corresponds to a REGISTERED plugin
      // (legacy JSON-stub templates don't have a component to dispatch to).
      if (selectedTemplateId && getTemplate(selectedTemplateId)) {
...
export default function KissGlobe() {
  ...
  return (
    <>
      ...
      {/* Kiss Reveal — dispatches to a registered template plugin when
          kiss.template_id points to one, otherwise falls back to the
          default cinematic KissRevealPopup. */}
      <AnimatePresence>
        {revealKiss && (() => {
          const tpl = getTemplate(revealKiss.template_id);
          if (tpl) {
