...
function RealisticDove({ flapDuration, flapDelay, color = '#FFFFFF' }: { flapDuration: number; flapDelay: number; color?: string }) {
  return (
    <svg viewBox="0 0 100 80" className="w-full h-full overflow-visible">
      {/* Cánh xa (Back Wing) */}
      <motion.path
        d="M 45,35 Q 25,5 5,15 Q 20,40 45,40 Z"
        fill={color}
        opacity={0.7}
      ...
      {/* Thân & Đuôi chim (Body & Tail) */}
      <path
        d="M 15,48 C 5,45 0,50 2,55 C 8,53 25,48 35,46 C 45,52 60,52 75,42 C 85,35 92,30 90,26 C 85,22 72,28 60,32 C 48,34 30,36 15,48 Z"
        fill={color}
      />
      {/* Mỏ chim */}
      <path d="M 89,27 L 96,29 L 88,31 Z" fill="#E67E22" />

      {/* Cánh gần (Front Wing) */}
      <motion.path
        d="M 50,38 Q 30,-5 8,8 Q 22,42 50,42 Z"
        fill={color}
...
const REAL_DOVE_GIF = "https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExOHY5b3JidmVpZjE2YThsYms1Ym5sM25tYnU0ZXRseDV3Mnl2eXJ2ciZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9cw/L1VDCscvO9M61pB5cR/giphy.gif";

export function DoveDelivery({ accent }: { accent: string }) {
  // Tạo đàn chim 18 con với vị trí, kích thước và đường bay bất quy tắc
  const flock = useMemo(() => {
    return Array.from({ length: 18 }).map((_, i) => {
      const seed = (i * 2654435761) >>> 0;
      const r = (n: number) => (((seed ^ (n * 0x9E3779B1)) >>> 0) % 10000) / 10000;

      // Chia tầng chiều sâu: gần (to, rõ) -> vừa -> xa (nhỏ, mờ)
      const tier: 'near' | 'mid' | 'far' = i < 5 ? 'near' : i < 12 ? 'mid' : 'far';

      const size = tier === 'near' ? 80 + r(3) * 25 : tier === 'mid' ? 50 + r(3) * 20 : 28 + r(3) * 12;
      const opacity = tier === 'near' ? 0.95 : tier === 'mid' ? 0.8 : 0.6;
      const blur = tier === 'far' ? 'blur(1.5px)' : 'none';

      const spreadY = tier === 'near' ? 220 : tier === 'mid' ? 340 : 460;
      const spreadX = tier === 'near' ? 280 : tier === 'mid' ? 450 : 600;

      // Tốc độ bay của từng con khác nhau hoàn toàn
      const flyDuration = 5.5 + (r(9) - 0.5) * 2.2;
      ...
      return {
        ...
        flyDuration,
        rotations: [r(10) * 12 - 6, r(11) * 18 - 9, r(12) * 12 - 6],
      };
    });
  }, []);
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      {flock.map((bird) => (
        <motion.div
          ...
          transition={{
            delay: 1.2 + (bird.id % 5) * 0.3, // Xuất hiện ngắt quãng tự nhiên
            duration: bird.flyDuration,
            ease: 'easeInOut',
            times: [0, 0.45, 0.88, 1],
          }}
        >
          <div style={{ transform: `translateX(${bird.offsetX}px)` }}>
            <img
              ...
              style={{
                // Đảo chiều quay mặt chim về phía trước nếu cần
                transform: 'scaleX(-1)',
              }}
            />
          </div>
      ...
      {/* ── CHIM CHÍNH MANG THƯ (HERO DOVE) ── */}
      <motion.div
        className="absolute top-1/2 w-28 h-28 z-10"
        initial={{ x: '-20vw', y: 0, opacity: 0 }}
        animate={{
          x: ['-20vw', '48vw', '110vw'],
          y: [15, -25, 10, -10],
      ...
      >
        <div className="relative w-full h-full" style={{ filter: `drop-shadow(0 0 20px ${accent}aa)` }}>
          <img
            src={REAL_DOVE_GIF}
            alt="hero dove"
            className="w-full h-full object-contain pointer-events-none"
          ...
          {/* Bức thư 💌 lắc lơ dưới chân chim */}
          <motion.div
            className="absolute bottom-2 right-4 text-3xl pointer-events-none"
            animate={{
              rotate: [-15, 20, -15],
              y: [0, 4, 0],
            }}
