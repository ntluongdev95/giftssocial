      ...
      <div className="max-w-lg lg:max-w-2xl mx-auto px-4 lg:px-8 py-4 pb-24 space-y-3">
        {isLoading ? (
          ...
          signals.map((s) => {
            ...
            return (
              <div key={s.id as string} className="rounded-2xl p-4" style={{ background: 'rgba(17,19,24,0.5)', border: '1px solid rgba(255,255,255,0.04)' }}>
                <div className="flex items-start gap-3">
                  <div className="h-11 w-11 rounded-xl flex items-center justify-center text-lg shrink-0" style={{ background: `${cfg.color}12`, border: `1px solid ${cfg.color}20` }}>
                    {cfg.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-white">{s.title as string}</h3>
                    {s.description && <p className="text-[11px] text-[#4a5068] mt-0.5 line-clamp-2">{s.description as string}</p>}
