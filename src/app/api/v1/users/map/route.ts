...
import {
  buildLocationVisibilityClause,
  buildLocationVisibilityReasonSelects,
  resolveVisibilityReason,
} from '@/lib/visibility';

/**
 * GET /api/v1/users/map?lat=...&lng=...&radius=100000
 *
 * Returns users visible to the current viewer, along with the reason each
 * one is visible (public / friend / circle / event / self), so the client
 * can render context-aware markers.
 *
...
export async function GET(req: NextRequest) {
  ...
  const reasons = buildLocationVisibilityReasonSelects('u', viewerId);
  const db = getDB();
  ...
  try {
    ...
    if (hasGeo) {
      ...
      sql = `SELECT u.id, u.display_name, u.username, u.avatar_url, u.location_lat, u.location_lng, u.city, u.trust_level, u.trust_score,
             ...
             LIMIT ?`;
      values.push(...reasons.params, ...visibility.params, radiusKm, limit);
    } else {
      sql = `SELECT u.id, u.display_name, u.username, u.avatar_url, u.location_lat, u.location_lng, u.city, u.trust_level, u.trust_score,
                    ${reasons.sql}
             FROM users u
             ...
             LIMIT ?`;
      values.push(...reasons.params, ...visibility.params, limit);
    }

    const { results: rows } = await db.prepare(sql).bind(...values).all<Record<string, unknown>>();

    const data = rows.map(r => {
      const ctx = resolveVisibilityReason(r, r.id as string, viewerId);
      return {
        id: r.id,
        ...
        visibility_reason: ctx.reason,
        shared_event_id: ctx.event_id || null,
        shared_circle_id: ctx.circle_id || null,
      };
    });
