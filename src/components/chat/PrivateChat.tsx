        ...
        >
          ...
          <div className="shrink-0 flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="flex items-center gap-3">
              {onBack && (
                <button onClick={onBack} className="h-8 w-8 rounded-lg flex items-center justify-center cursor-pointer" style={{ background: 'rgba(255,255,255,0.05)' }}>
              ...
              <div>
                ...
                {subtitle && <p className="text-[10px] text-[#4a5068]">{subtitle}</p>}
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
            {messages.length === 0 && (
              <div className="flex flex-col items-center gap-2 py-12 text-center">
                ...
                <p className="text-xs text-[#4a5068]">Send a message to start the conversation</p>
              </div>
            )}
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex gap-2.5 ${isMe ? 'flex-row-reverse' : ''}`}>
                  <div className="h-8 w-8 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 overflow-hidden" style={{ background: isMe ? '#00d4ff' : '#3B82F6', color: 'white' }}>
                    {(() => {
                      const avatar = isMe ? (myAvatar || (msg as unknown as Record<string, unknown>).sender_avatar as string) : ((msg as unknown as Record<string, unknown>).sender_avatar as string || '');
