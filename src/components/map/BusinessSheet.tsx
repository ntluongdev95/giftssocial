  ...
  return (
    <><AnimatePresence>
      ...
      >
        ...
        >
          {/* ── Cover Image ──────────────────────────────── */}
          {!!(biz as unknown as Record<string, unknown>).cover_image && (
            <div className="h-36 overflow-hidden shrink-0">
              <img src={(biz as unknown as Record<string, unknown>).cover_image as string} alt="" className="h-full w-full object-cover" />
            </div>
          )}

          {/* ── Header ────────────────────────────────────── */}
          <div className="relative px-5 pt-5 pb-4">
            {!(biz as unknown as Record<string, unknown>).cover_image && <div className="absolute inset-x-0 top-0 h-28 opacity-40 rounded-t-3xl" style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.25), rgba(0,212,255,0.1))' }} />}
                    ...
                    <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-white/[0.02] transition-colors" style={{ borderBottom: i < services.length - 1 ? '1px solid rgba(255,255,255,0.03)' : undefined }}>
                      ...
                      {biz.booking_enabled && (
                        <button
                          ...
                          className="ml-2 rounded-lg px-2.5 py-1 text-[9px] font-semibold cursor-pointer"
          ...
          <div className="shrink-0 px-5 pt-3 pb-[calc(env(safe-area-inset-bottom,0px)+16px)] lg:pb-4 flex gap-2" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            {biz.booking_enabled && (
              <button onClick={() => openBooking()} className="flex-1 rounded-xl py-3 text-sm font-semibold flex items-center justify-center gap-2 cursor-pointer" style={{ background: '#00d4ff', color: '#0a0b0f' }}>
