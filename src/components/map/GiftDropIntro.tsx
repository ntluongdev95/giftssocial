...
export default function GiftDropIntro({
  ...
  // Dense, slow rain — 45 items mixing gifts, letters, hearts, and
  // "LOVE" text tokens. Random left / rotation / delay stay stable
  // across renders. Delays spread across the whole intro so the sky
  // keeps filling with new items throughout.
  ...
  // Deep-space stars — 220 dots covering the ENTIRE sky (0-100% top/left)
  // in three brightness tiers so the field reads as natural rather than
  // uniformly bright. Bigger stars twinkle more slowly.
  const stars = useMemo(() => (
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {/* Vehicle-specific SCENE backdrop — different visual world per
          vehicle so the intro reads as "delivery in progress" on the
          right kind of setting:
             dove  → starry night sky with a couple watching from below
             car   → nighttime highway in one-point perspective
             plane → high-altitude sky with drifting clouds
          The vehicle animation itself renders on top of this scene. */}
      {vehicle === 'dove'  && <StarrySkyScene stars={stars} />}
      {vehicle === 'car'   && <HighwayScene />}
      {vehicle === 'plane' && <NightCityScene />}

      {/* Dense drifting rain — gifts + hearts + "LOVE" text */}
      {falling.map(g => (
        <motion.div
          key={g.id}
          ...
          style={{
            left: `${g.left}%`, top: '-8%',
            fontSize: g.kind === 'text' ? Math.max(18, g.size * 0.9) : g.size,
            color: g.kind === 'text' ? '#ec4899' : undefined,
            letterSpacing: g.kind === 'text' ? '0.15em' : undefined,
            filter: g.kind === 'text'
              ? 'drop-shadow(0 0 8px rgba(236,72,153,0.7)) drop-shadow(0 2px 6px rgba(0,0,0,0.6))'
              : 'drop-shadow(0 4px 10px rgba(0,0,0,0.7))',
            opacity: 0.95,
          }}
          initial={{ y: 0, rotate: 0, opacity: 0 }}
          animate={{ y: '110vh', rotate: g.rotate, opacity: [0, 0.95, 0.95, 0] }}
          transition={{ duration: g.duration, delay: g.delay, ease: 'linear' }}
        >
          {g.content}
        </motion.div>
      ))}

      {/* Vehicle "delivery" animation (phase 2 — starts ~2.6s in) */}
...
// Dove default — starry night sky with the couple watching from below.
...
// Plane — flying over a NIGHT CITY. Deep-navy sky above, sprinkled
// with stars; the city sits along the bottom third as a silhouette
// bristling with warm window lights. Two skyline layers parallax at
// different speeds (far row slower than near row) for depth. No
// clouds — the intent is "cruising above a lit city at night".
function NightCityScene() {
  return (
    <>
      {/* Deep night sky */}
      <div className="absolute inset-0" style={{
        background: 'linear-gradient(180deg, #05070d 0%, #0a0e1e 45%, #10162d 75%, #1a1f38 100%)',
      }} />

      {/* Stars sprinkled across the top 70% */}
      ...
      {/* Warm haze glowing up from the city — like light pollution */}
      <div className="absolute inset-x-0" style={{
        bottom: '18%', height: '15%',
        background: 'linear-gradient(180deg, transparent, rgba(255,180,80,0.16) 50%, rgba(255,140,60,0.28) 100%)',
        filter: 'blur(4px)',
      }} />

      {/* FAR skyline — slow parallax right→left, dimmer + smaller */}
      <motion.div
        className="absolute inset-x-0"
        style={{ bottom: '18%', height: '14%' }}
        initial={{ x: 0 }}
        animate={{ x: '-40vw' }}
        transition={{ duration: 110, repeat: Infinity, ease: 'linear' }}
      >
        <svg viewBox="0 0 1000 70" preserveAspectRatio="xMinYMax meet" className="w-[200%] h-full block">
          <CitySkyline windowOpacity={0.55} />
        </svg>
      </motion.div>

      {/* NEAR skyline — faster parallax, taller, brighter windows */}
      <motion.div
        className="absolute inset-x-0"
      ...
      >
        <svg viewBox="0 0 1000 120" preserveAspectRatio="xMinYMax meet" className="w-[200%] h-full block">
          <CitySkyline near windowOpacity={1} />
        </svg>
      </motion.div>
    </>
  );
...
function DoveDelivery({ accent }: { accent: string }) {
  return (
    <motion.div
      ...
      animate={{
        // -30vw → 50vw (middle hero moment) → 92vw (near right edge)
        x: ['-30vw', '50vw', '92vw'],
        opacity: [0, 1, 1],
      }}
      transition={{
        delay: 2.6,
        duration: 6.4,
        // Reach the middle at ~38% of the timeline, spend the remaining
        // 62% drifting onward — hero-moment mid-screen then a leisurely
        // departure toward the right edge.
        times: [0, 0.38, 1],
        ease: 'easeInOut',
      }}
    >
      {/* Static wrapper: centering + FLIP live here so framer-motion
          animations on children don't overwrite the scaleX(-1). */}
      <div style={{ transform: 'translateX(-50%) scaleX(-1)' }}>
        <motion.div
          ...
          style={{
            filter: `drop-shadow(0 6px 16px rgba(0,0,0,0.6)) drop-shadow(0 0 30px ${accent}55)`,
            display: 'block',
        ...
        {/* Un-flip the letter with a static div — the outer wrapper's
            scaleX(-1) applies to everything inside otherwise, mirroring
            the 💌 glyph. Framer-motion on the inner div can safely
            animate `rotate` without touching this scale. */}
